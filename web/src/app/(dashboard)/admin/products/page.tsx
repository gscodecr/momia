...
export default function AdminProducts() {
  ...
  const fetchOrders = async () => {
    try {
      const res = await fetch('http://127.0.0.1:8001/admin/orders', {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      if (res.ok) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await fetch('http://127.0.0.1:8001/products/upload', {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
        body: formData
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingProductId ? `http://127.0.0.1:8001/products/${editingProductId}` : 'http://127.0.0.1:8001/products/';
  ...
  const handleDeliverOrder = async (orderId: number) => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:8001/admin/orders/${orderId}/deliver`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      if (res.ok) {
                ...
                <div className="border-2 border-dashed border-white/20 rounded-xl p-4 text-center hover:border-[var(--primary)] transition-colors relative flex flex-col items-center justify-center overflow-hidden h-24">
                  {imageUrl ? (
                    <div className="flex items-center gap-3 w-full justify-center">
                       <img src={`http://127.0.0.1:8001${imageUrl}`} alt="preview" className="h-16 w-16 object-cover rounded-lg border border-white/10" />
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-zinc-800 rounded flex items-center justify-center overflow-hidden">
                            {p.image_url ? <img src={`http://127.0.0.1:8001${p.image_url}`} className="object-cover w-full h-full" alt="" /> : <Package size={20} className="opacity-50"/>}
                          </div>
                          <div>
