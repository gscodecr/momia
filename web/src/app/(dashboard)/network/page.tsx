...
export default function NetworkPage() {
  ...
  const fetchItems = async () => {
    try {
      ...
      const res = await fetch('http://127.0.0.1:8001/network/', {
        headers: {
          ...
          {filteredItems.map((item) => (
            <div key={item.id} onClick={() => openDetail(item)} className="p-4 hover:bg-white/5 cursor-pointer transition-colors flex gap-4">
              <div className="w-20 h-20 rounded-lg bg-[var(--background)] overflow-hidden flex-shrink-0 border border-white/5">
                <img src={item.photo_url ? `http://127.0.0.1:8001${item.photo_url}` : 'https://via.placeholder.com/400x300?text=Sin+Imagen'} alt={item.title} className="w-full h-full object-cover" />
              </div>
        ...
        <div className="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl relative">
            ...
            <div className="h-64 bg-[var(--background)] relative w-full">
              <img src={selectedItem.photo_url ? `http://127.0.0.1:8001${selectedItem.photo_url}` : 'https://via.placeholder.com/400x300?text=Sin+Imagen'} alt={selectedItem.title} className="w-full h-full object-cover" />
