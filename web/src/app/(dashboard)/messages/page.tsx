...
export default function MessagesPage() {
      ...
      ws.current.onmessage = (event) => {
        ...
        if (selectedContactRef.current && (data.sender_id === selectedContactRef.current.id || data.sender_id === user.id)) {
          ...
          if (data.sender_id === selectedContactRef.current.id) {
            fetch(`http://127.0.0.1:8001/chat/read/${data.sender_id}`, {
              method: 'PUT',
              headers: { 'Authorization': `Bearer ${token}` }
            }).then(() => {
  ...
  const fetchContacts = async (token?: string | null) => {
    ...
    try {
      const res = await fetch('http://127.0.0.1:8001/chat/contacts', {
  ...
  const fetchHistory = async (targetId: number, currentOffset: number = 0) => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:8001/chat/history/${targetId}?limit=50&offset=${currentOffset}`, {
        headers: { 'Authorization': `Bearer ${authToken}` }
      });
      const data = await res.json();
      if (Array.isArray(data)) {
  ...
  const getAvatarUrl = (url: string | null) => {
    if (!url) return null;
    return url.startsWith('http') ? url : `http://127.0.0.1:8001${url}`;
  };
  ...
  const handleBroadcast = async () => {
    ...
    try {
      ...
      await fetch('http://127.0.0.1:8001/chat/broadcast', {
        method: 'POST',
        headers: { 
