...
export default function AthleteWorkouts() {
  ...
  const handleToggleComplete = async (workout: any) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || process.env.NEXT_PUBLIC_API_URL || process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8001" + ""}/workouts/${workout.id}`, {
        method: 'PUT',
        headers: {
