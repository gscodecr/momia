...
export default function AthleteBilling() {
  ...
  const handleToggleAutoPay = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setAutoPay(newValue);
    try {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8001"}/payments/auto-pay?auto_pay=${newValue}`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const handleTiloPaySimulate = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8001"}/payments/tilopay/simulate?amount=${amount}&description=${desc}`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      if (res.ok) {
  ...
  const handleSinpeSubmit = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8001"}/payments/report-sinpe?amount=${amount}&description=${desc}`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
        body: formData
      });
